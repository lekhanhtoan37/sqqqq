    ...
    */
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let indexPath = tableView.indexPathForSelectedRow {
            //if let TVC = segue.destination as? ViewController {
                // config prepare initials
                Singleton.shared.stringg = dataString[indexPath.row]
    ...
    @IBAction func goBack(_ sender : UIStoryboardSegue) {
        if let indexPath = tableView.indexPathForSelectedRow {
            ...
            dataString[indexPath.row] = Singleton.shared.stringg ?? ""
